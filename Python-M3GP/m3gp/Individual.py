...
from sklearn.metrics import accuracy_score, f1_score, cohen_kappa_score, mean_squared_error, mean_absolute_error
...
class Individual:
	...
	def createModel(self):
		if self.model_name == "MahalanobisDistanceClassifier":
			...
			return LinearRegression() #max depth iniziale 6
			#return RandomForestRegressor(random_state=42, n_estimators=10, max_depth=6)
	...
	def getFitness(self, tr_x = None, tr_y = None):
		...
		if self.fitness is None:
			...
			if self.fitnessType == "MSE":
				...
				#mse = mean_squared_error(self.trainingPredictions, self.training_Y)
				self.fitness = mse 

			if self.fitnessType == "MAE":
				self.fit(self.training_X, self.training_Y)
				self.getTrainingPredictions()
				mae = -1 * mean_absolute_error(self.trainingPredictions, self.training_Y)
				self.fitness = mae 
	...
	def getTrainingMeasure(self):
		...
		if self.fitnessType == "MSE":
			...
			#return mean_squared_error(self.trainingPredictions, self.training_Y)
		
		if self.fitnessType == "MAE":
			self.getTrainingPredictions()
			return -1 * mean_absolute_error(self.trainingPredictions, self.training_Y)
	...
	def getTestMeasure(self, test_X, test_Y):
		...
		if self.fitnessType == "MSE":
			self.getTestPredictions(test_X)
			#return mean_squared_error(self.testPredictions, test_Y)
			return -1 * mean_squared_error(self.testPredictions, test_Y)
		
		if self.fitnessType == "MAE":
			self.getTestPredictions(test_X)
			return -1 * mean_absolute_error(self.testPredictions, test_Y)
	...
	def getMAE(self, X,Y,pred=None):
		...
		return -1 * mean_absolute_error(pred, Y)
